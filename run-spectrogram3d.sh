#!/bin/bash
# Script to run OpenLase 3D spectrogram with X11 forwarding
...
if [[ "$OSTYPE" == "darwin"* ]]; then
    ...
    echo "Starting 3D spectrogram with DISPLAY=$IP:0"
    docker run --rm -it \
        -e DISPLAY="$IP:0" \
    ...
    echo "Starting 3D spectrogram with DISPLAY=$DISPLAY"
    docker run --rm -it \
        -e DISPLAY="$DISPLAY" \
        -v /tmp/.X11-unix:/tmp/.X11-unix \
        --network host \
